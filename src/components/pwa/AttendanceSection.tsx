...
export const AttendanceSection: React.FC<AttendanceSectionProps> = ({
    ...
    const [activeSubTab, setActiveSubTab] = useState<'calendar' | 'ranking'>('calendar');
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-5 duration-700">
            {/* Tabs Selector */}
            <div className="flex p-1 bg-white/5 rounded-2xl border border-white/5 mx-1">
                <button
                    onClick={() => setActiveSubTab('calendar')}
                    className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeSubTab === 'calendar' ? 'bg-white/10 text-white' : 'text-white/30'}`}
                >
                    Mi Calendario
                </button>
                <button
                    onClick={() => setActiveSubTab('ranking')}
                    className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeSubTab === 'ranking' ? 'bg-white/10 text-white' : 'text-white/30'}`}
                >
                    Ranking del Mes
                </button>
            </div>
                ...
                <div className="animate-in fade-in zoom-in-95 duration-500">
                    {loadingLeaderboard ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-4">
                            <div className="w-8 h-8 border-4 border-t-transparent animate-spin rounded-full" style={{ borderColor: `${primaryColor}40`, borderTopColor: 'transparent' }} />
                            <p className="text-[10px] text-white/20 font-black uppercase tracking-widest">Cargando Ranking...</p>
                        </div>
                    ) : leaderboard ? (
                        <RankingSection
                            top10={leaderboard.top10}
                            myPosition={leaderboard.myPosition}
                            primaryColor={primaryColor}
                            monthName={leaderboard.monthName}
                        />
                    ) : (
                        <div className="text-center py-20 text-white/20 text-xs font-bold">No hay datos de ranking disponibles</div>
                    )}
                </div>
            )}
        </div>
